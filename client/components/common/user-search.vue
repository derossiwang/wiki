...
v-dialog(v-model="dialogOpen", max-width="650")
  v-card
    ...
    v-card-chin
      v-spacer
      v-btn(text, @click="close", :disabled="loading") {{ $t('common:actions.cancel') }}
</template>
...
export default {
  ...
  methods: {
    ...
    searchFilter(item, queryText, itemText) {
      return (
        _.includes(_.toLower(item.email), _.toLower(queryText)) ||
        _.includes(_.toLower(item.name), _.toLower(queryText))
      )
    }
  },
  apollo: {
