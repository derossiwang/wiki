...
module.exports = {
  async activate() {
    if (WIKI.config.db.type !== "postgres") {
      throw new WIKI.Error.SearchActivationFailed(
        "Must use PostgreSQL database to activate this engine!"
      );
    }
  },
  async deactivate() {
  ...
  async init() {
    ...
    if (!wordsExists) {
      ...
      await WIKI.models.knex.raw(
        `CREATE INDEX "pageWords_idx" ON "pagesWords" USING GIN (word gin_trgm_ops)`
      );
    }
  ...
  async rebuild() {
    ...
    await pipeline(
      WIKI.models.knex
        .column("path", "localeCode", "title", "description", "render")
        .select()
        .from("pages")
        .where({
          isPublished: true,
